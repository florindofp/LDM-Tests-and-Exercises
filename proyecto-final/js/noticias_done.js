...
const divParent = document.getElementById("parent_template");
const template = document.getElementById("card_col_template");
const articles = noticias.articles;

articles.forEach((item, index) => {
    const newCol = template.cloneNode(true);
    newCol.style.display = "";
    newCol.removeAttribute("id");

    newCol.querySelector(".card-title").textContent = item.title;
    newCol.querySelector(".card-text").textContent = item.nombre;
    newCol.querySelector(".card-img-top").src = item.image;
    newCol.querySelector(".card-link").href = item.url;

    divParent.appendChild(newCol);
});